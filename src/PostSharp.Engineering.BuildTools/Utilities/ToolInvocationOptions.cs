    ...
    ToolInvocationRetry? Retry = null )
{
    ...
    public ImmutableArray<Regex> ErrorPatterns { get; init; } = ImmutableArray.Create( new Regex( "\\: error " ) );

    public ImmutableArray<Regex> WarningPatterns { get; init; } = ImmutableArray.Create( new Regex( "\\: warning " ) );
